...
// Import individual tool creators as needed
...
export function createServer() {
  ...
  // Tools are registered directly in this file
  
  // Register change_keyword tool
  server.tool('change_keyword',
    'Change the keyword of an existing short URL',
    {
      ...
      url: z.string().optional().describe('Optional URL (if not provided, will use the URL from oldshorturl)'),
      title: z.string().optional().describe('Optional new title ("keep" to keep existing, "auto" to fetch from URL)')
    },
    async ({ oldshorturl, newshorturl, url, title }) => {
      try {
        ...
        if (result.status === 'success' || result.message === 'success: updated' || result.statusCode === 200) {
          ...
          return createMcpResponse(responseData);
        } else {
          return createMcpResponse({
            status: 'error',
            message: result.message || 'Failed to change keyword',
            code: result.code || 'error:unknown'
          });
        }
      } catch (error) {
        return createMcpResponse({
          status: 'error',
          message: error.message || 'An error occurred while changing the keyword',
          code: error.code || 'error:unknown'
        });
      }
    }
  );
